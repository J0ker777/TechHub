...
async function showGoogle(){googleBox.hidden=false;nameForm.hidden=true;authTitle.textContent='Увійдіть через Google';try{await loadGoogle();google.accounts.id.initialize({client_id:AUTH.clientId,callback:googleCredential});google.accounts.id.renderButton(document.querySelector('#google-button'),{theme:'outline',size:'large',text:'signin_with',shape:'pill',width:300,logo_alignment:'left'});}catch{authError('Не вдалося завантажити Google Sign-In. Перевірте підключення.');}}
async function authInit(){try{const raw=localStorage.getItem(AUTH.key);if(raw)auth=JSON.parse(raw);}catch{}if(!auth?.sessionToken){showGoogle();return;}try{const response=await fetch(AUTH.endpoint+'/auth/me',{headers:{Authorization:'Bearer '+auth.sessionToken}});if(!response.ok)throw Error();const current=await response.json();auth={...auth,expiresAt:current.expiresAt,user:current.user};authSave();showName();}catch{auth=null;localStorage.removeItem(AUTH.key);showGoogle();}}
