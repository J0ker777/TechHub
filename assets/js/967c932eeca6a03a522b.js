...
async function showGoogle(){googleBox.hidden=false;nameForm.hidden=true;authTitle.textContent='Увійдіть через Google';try{await loadGoogle();google.accounts.id.initialize({client_id:AUTH.clientId,callback:googleCredential});google.accounts.id.renderButton(document.querySelector('#google-button'),{theme:'outline',size:'large',text:'signin_with',shape:'rectangular'});}catch{authError('Не вдалося завантажити Google Sign-In. Перевірте підключення.');}}
